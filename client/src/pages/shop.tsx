...
// removed: import { BOOKS } from "@/lib/data";
...
const SHEET_CSV_URL =
  "https://docs.google.com/spreadsheets/d/1-6wJ99DtiM4AX-fMJZI5ko6MsnqkPSRbiaobrRHsq0M/export?format=csv";
...
export default function Shop() {
  ...
  useEffect(() => {
    ...
    fetch(SHEET_CSV_URL)
      ...
      .then((text) => {
        ...
        // Normalize headers (case-insensitive) and map to the book shape used in the page
        const normalized = rows.map((r, idx) => {
          // helper to get a value by header name variants
          const get = (names: string[]) => {
            for (const n of names) {
              if (n in r && r[n] !== undefined && r[n] !== "") return r[n];
            }
            // try case-insensitive
            const foundKey = Object.keys(r).find(
              (k) => k.toLowerCase() === names[0].toLowerCase(),
            );
          ...
          const ageRaw = get(["Age", "age", "Age Group", "ageGroup"]);
          const type = get(["Type", "type"]);
          const condition = get(["Condition", "condition"]);
          const priceRaw = get(["Price", "price"]);
          const availability = get(["Availability", "availability"]);
          const image = get(["image_link", "Image", "image", "image_link"]);

          const price = Number(priceRaw.replace(/[^\d.-]/g, "")) || 0;
          const ageGroup = ageRaw || "0-3";
          ...
          return {
            ...
            image: image || "/placeholder-book.jpg", // adjust placeholder path if needed
          };
        });
  ...
  }, []); // load once on mount (reads CSV each time page is loaded)

  const filteredBooks = books.filter((book) => {
    const ageMatch =
      activeFilter === "all" || String(book.ageGroup) === activeFilter;
          ...
          {loading ? (
            ...
            filteredBooks.map((book) => {
              ...
              return (
                <div
                  key={book.id}
                  className="group bg-white rounded-[3rem] overflow-hidden shadow-[0_12px_0_0_var(--border)] hover:translate-y-[-8px] transition-all border-4 border-border"
                >
                  <div className="aspect-[3/4] bg-muted relative overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4">
                      <span
                        className={`text-xs font-black uppercase tracking-widest px-4 py-2 rounded-full border-2 border-white shadow-lg ${
                  ...
                  <div className="p-8">
                    <h3 className="font-black text-2xl mb-2 line-clamp-1 group-hover:text-primary transition-colors">
                      {book.title}
                    </h3>
                    <p className="text-muted-foreground text-sm mb-6 font-bold">
                      by {book.author}
                    </p>
                    ...
                    <div className="flex items-center justify-between mt-auto">
                      ...
                      <Button
                        ...
                        className={cn(
                          ...
                          !isInCart &&
                            "shadow-[0_4px_0_0_rgba(0,0,0,0.15)] bg-primary text-white hover:bg-primary/90",
                        )}
